        ...
        {checks.map((check) => (
          ...
          >
            ...
            <span>
              ...
              {check.note ? (
                <span className="block text-slate-500">{check.note}</span>
              ) : null}
            </span>
          </li>
        ))}
      ...
      <main className="bg-slate-50">
        <Container className="py-12">
          ...
          <Link
            ...
            className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-highland focus:outline-none focus:ring-2 focus:ring-highland focus:ring-offset-2"
          >
  ...
  return (
    <main className="bg-slate-50">
      <Container className="py-8 sm:py-10">
        <Link
          href="/businesses/manage"
          className="inline-flex items-center gap-2 text-sm font-semibold text-highland focus:outline-none focus:ring-2 focus:ring-highland focus:ring-offset-2"
        >
          <ArrowLeft className="h-4 w-4" aria-hidden="true" />
          My Businesses
        </Link>
        <section className="mt-5 rounded-lg border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <p className="text-sm font-semibold uppercase tracking-wide text-highland">
                Business workspace · {business.currentMember.role.toLowerCase()}
              </p>
            ...
            <div className="flex flex-wrap gap-2 text-xs font-semibold">
              <span
                className={`rounded-md px-2.5 py-1 ${statusTone(business.status)}`}
              >
                {business.status}
              </span>
          ...
          {business.status === 'SUSPENDED' ? (
            <p className="mt-5 rounded-md border border-red-200 bg-red-50 p-4 text-sm text-red-900">
              This business is suspended. History remains available, but no new
              booking setup action is offered here.
            </p>
          ) : null}
          {business.verificationSummary === 'NOT_SUBMITTED' ? (
          ...
          <aside className="space-y-6">
            ...
            <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
              <div className="flex items-center gap-2">
                <ShieldCheck
                  className="h-5 w-5 text-highland"
                  aria-hidden="true"
                />
